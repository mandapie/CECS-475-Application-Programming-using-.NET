    ...
    public partial class frmCustomerMaintenance : Form
    {
        ...
        private void GetCustomer(int CustomerID)
        {
            try
            {
                ...
                var getCustomer =
                    (from customer in MMABooks.mmabooksEntities.Customers
                    where customer.CustomerID == CustomerID
                    select customer);
                // and store the Customer object in the class variable.
                selectedCustomer = getCustomer.Single();
                ...
                else
                {
                    ...
                    if (!MMABooks.mmabooksEntities.Entry(selectedCustomer).Reference("State").IsLoaded)
                        MMABooks.mmabooksEntities.Entry(selectedCustomer).Reference("State").Load();
        ...
        {
            DialogResult result = MessageBox.Show("Delete " + selectedCustomer.Name + "?", "Confirm Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    // Update the database.
                    MMABooks.mmabooksEntities.Customers.Remove(selectedCustomer);
                ...
                catch (DbUpdateConcurrencyException ex)
                {
                    ...
                    if (MMABooks.mmabooksEntities.Entry(selectedCustomer).State == EntityState.Detached)
                    {
