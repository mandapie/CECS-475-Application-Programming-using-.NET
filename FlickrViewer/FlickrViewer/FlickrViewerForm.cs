   ...
   public partial class FlickrViewerForm : Form
   {
      ...
      private async void imagesListBox_SelectedIndexChanged(object sender, EventArgs e)
      {
         if (imagesListBox.SelectedItem != null)
         {
            ...
            Byte[] imageBytes = await imgClient.DownloadDataTaskAsync(selectedURL);

            //Create a MemoryStream object from imagesBytes
            MemoryStream imageMStream = new MemoryStream(imageBytes);
            ...
            pictureBox.Image = Image.FromStream(imageMStream);
         } // end if
